...
type (
	Repository interface {
		...
		Create(ctx context.Context, e *entities.ItemThumbnail) (*entities.ItemThumbnail, error)
...
func (r *repository) List(ctx context.Context, offset, limit int, sortDir, sortBy *string) (*[]entities.ItemThumbnail, int64, error) {
	...
	var collection []entities.ItemThumbnail
...
func (r *repository) Create(ctx context.Context, e *entities.ItemThumbnail) (*entities.ItemThumbnail, error) {
	if e.ItemID == 0 {
...
func (r *repository) Update(ctx context.Context, e *entities.ItemThumbnail) (*entities.ItemThumbnail, error) {
	...
	if e.FileID != 0 {
		updateData["file_id"] = e.FileID
	}

	if err := r.db.WithContext(ctx).Model(&entities.ItemThumbnail{}).Where("item_id = ?", e.ItemID).Updates(updateData).Error; err != nil {
		return nil, err
	}
...
func (r *repository) Delete(ctx context.Context, itemID uint) error {
	res := r.db.WithContext(ctx).Model(&entities.ItemThumbnail{}).Where("item_id = ?", itemID).Delete(&entities.ItemThumbnail{})
	if res.Error != nil {
		return res.Error
	}
