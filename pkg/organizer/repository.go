...
func (r *repository) Get(ctx context.Context, id uint) (*entities.Organizer, error) {
	...
	if err := r.db.WithContext(ctx).Model(entities.Organizer{}).Preload("Auctions.Item").Preload("Auctions.Item.ObjectType").Preload("Auctions.Item.File").Preload("Auctions.PIC").Preload("Auctions.Organizer").First(&ot, id).Error; err != nil {
